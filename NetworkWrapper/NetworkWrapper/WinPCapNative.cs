...
namespace NetworkWrapper
{
    ...
    public class WinPCapNative
    {
        ...
        internal const int MAX_PACKET_SIZE = 0x10000;
        internal const int MODE_CAPT = 0;
        internal const int MODE_STAT = 1;
        internal const int PCAP_BUF_SIZE = 0x400;
        internal const int PCAP_ERRBUF_SIZE = 0x100;
        internal const uint PCAP_IF_LOOPBACK = 1;
        ...
        [Obsolete("Deprecated due to incompatibilities between the C Runtime used to compile WinPcap"), DllImport("wpcap.dll", CallingConvention=CallingConvention.Cdecl, CharSet=CharSet.Ansi)]
        ...
        [Obsolete("This function does not work in current version of WinPcap."), DllImport("packet.dll", CallingConvention=CallingConvention.Cdecl, CharSet=CharSet.Ansi)]
        ...
        [Obsolete("Use pcap_findalldevs() or pcap_findalldevs_ex() instead."), DllImport("packet.dll", CallingConvention=CallingConvention.Cdecl, CharSet=CharSet.Ansi)]
        internal static extern string pcap_lookupdev(string errbuf);
        [Obsolete("Use pcap_findalldevs() or pcap_findalldevs_ex() instead."), DllImport("wpcap.dll", CallingConvention=CallingConvention.Cdecl, CharSet=CharSet.Ansi)]
        ...
        [DllImport("wpcap.dll", CallingConvention=CallingConvention.Cdecl, CharSet=CharSet.Ansi)]
        internal static extern int pcap_sendpacket(IntPtr p, byte[] buf, int size);
        [DllImport("wpcap.dll", CallingConvention=CallingConvention.Cdecl, CharSet=CharSet.Ansi)]
        ...
        public delegate void PacketArrivalEventHandler(object sender, PcapHeader p, byte[] s);
        ...
        {
            public WinPCapNative.timeval ts;
            public int caplen;
            public int len;
        }
